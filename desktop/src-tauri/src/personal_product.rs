...
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct ReflectionSearchRequest { session_token: Option<String>, query: String, state: String, limit: u32, offset: u32 }
...
#[tauri::command] fn desktop_reflection_search(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: ReflectionSearchRequest) -> Result<Value, String> { bounded(&[&request.query])?; if !matches!(request.state.as_str(), "ALL" | "ACTIVE" | "CLOSED") || !(1..=50).contains(&request.limit) { return Err("INVALID_PAYLOAD".to_string()); } personal_call(&window, &state, "reflection.search", request.session_token.as_deref(), json!({"query": request.query, "state": request.state, "limit": request.limit, "offset": request.offset})) }
