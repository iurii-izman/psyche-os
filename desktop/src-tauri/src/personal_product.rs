...
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct InterviewConsentRequest { session_token: Option<String>, interview_session_id: String, include_sleep: bool }
...
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct ChangeControlRequest { session_token: Option<String>, plan_id: String, action: String }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct ChangeObserveRequest { session_token: Option<String>, plan_id: String, content: String, signal: Option<String> }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct ChangeObservationPolicyRequest { session_token: Option<String>, plan_id: String, enabled: bool }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct ChangeReviewRequest { session_token: Option<String>, plan_id: String }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct SleepInboxRequest { session_token: Option<String>, inbox_path: String }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct SleepHistoryRequest { session_token: Option<String>, days: u8 }
#[derive(Deserialize)] #[serde(rename_all = "camelCase", deny_unknown_fields)] struct SleepDeleteRequest { session_token: Option<String>, external_record_id: String }
...
#[tauri::command] fn desktop_ai_interview_external_policy(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: InterviewPolicyRequest) -> Result<Value, String> { personal_call(&window, &state, "ai.interview.external_policy", request.session_token.as_deref(), json!({"enabled": request.enabled})) }
...
#[tauri::command] fn desktop_ai_interview_grant_consent(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: InterviewConsentRequest) -> Result<Value, String> { bounded(&[&request.interview_session_id])?; personal_call(&window, &state, "ai.interview.grant_consent", request.session_token.as_deref(), json!({"interview_session_id": request.interview_session_id, "include_sleep": request.include_sleep})) }
interview_session_command!(desktop_ai_interview_revoke_consent, "ai.interview.revoke_consent"); interview_session_command!(desktop_ai_interview_first_question, "ai.interview.first_question"); interview_session_command!(desktop_ai_interview_get, "ai.interview.get");
...
#[tauri::command] fn desktop_ai_change_list(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SessionRequest) -> Result<Value, String> { personal_call(&window, &state, "ai.change.list", request.session_token.as_deref(), json!({})) }
#[tauri::command] fn desktop_ai_change_control(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: ChangeControlRequest) -> Result<Value, String> { bounded(&[&request.plan_id, &request.action])?; personal_call(&window, &state, "ai.change.control", request.session_token.as_deref(), json!({"plan_id": request.plan_id, "action": request.action})) }
#[tauri::command] fn desktop_ai_change_observe(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: ChangeObserveRequest) -> Result<Value, String> { bounded(&[&request.plan_id])?; bounded_turn(&request.content)?; personal_call(&window, &state, "ai.change.observe", request.session_token.as_deref(), json!({"plan_id": request.plan_id, "content": request.content, "signal": request.signal})) }
#[tauri::command] fn desktop_ai_change_allow_observations(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: ChangeObservationPolicyRequest) -> Result<Value, String> { bounded(&[&request.plan_id])?; personal_call(&window, &state, "ai.change.allow_observations", request.session_token.as_deref(), json!({"plan_id": request.plan_id, "enabled": request.enabled})) }
#[tauri::command] fn desktop_ai_change_start_review(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: ChangeReviewRequest) -> Result<Value, String> { bounded(&[&request.plan_id])?; personal_call(&window, &state, "ai.change.start_review", request.session_token.as_deref(), json!({"plan_id": request.plan_id})) }
#[tauri::command] fn desktop_sleep_source_status(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SessionRequest) -> Result<Value, String> { personal_call(&window, &state, "external.source.status", request.session_token.as_deref(), json!({})) }
#[tauri::command] fn desktop_sleep_configure_inbox(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SleepInboxRequest) -> Result<Value, String> { bounded(&[&request.inbox_path])?; personal_call(&window, &state, "external.source.configure", request.session_token.as_deref(), json!({"inbox_path": request.inbox_path})) }
#[tauri::command] fn desktop_sleep_scan(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SessionRequest) -> Result<Value, String> { personal_call(&window, &state, "external.scan", request.session_token.as_deref(), json!({})) }
#[tauri::command] fn desktop_sleep_history(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SleepHistoryRequest) -> Result<Value, String> { if !(1..=30).contains(&request.days) { return Err("INVALID_PAYLOAD".to_string()); } personal_call(&window, &state, "sleep.history", request.session_token.as_deref(), json!({"days": request.days})) }
#[tauri::command] fn desktop_sleep_delete_record(window: WebviewWindow, state: tauri::State<'_, DesktopState>, request: SleepDeleteRequest) -> Result<Value, String> { bounded(&[&request.external_record_id])?; personal_call(&window, &state, "sleep.delete_record", request.session_token.as_deref(), json!({"external_record_id": request.external_record_id})) }
...
pub fn run() {
    tauri::Builder::default().manage(DesktopState { sidecar: Mutex::new(None) }).invoke_handler(tauri::generate_handler![
        ...
        desktop_ai_interview_status, desktop_ai_interview_policy, desktop_ai_interview_external_policy, desktop_ai_interview_source_policy, desktop_ai_interview_start, desktop_ai_interview_list, desktop_ai_interview_grant_consent, desktop_ai_interview_revoke_consent, desktop_ai_interview_first_question, desktop_ai_interview_submit, desktop_ai_interview_retry, desktop_ai_interview_control, desktop_ai_interview_get, desktop_ai_interview_disclosure, desktop_ai_model_list, desktop_ai_model_correct, desktop_ai_change_list, desktop_ai_change_control, desktop_ai_change_observe, desktop_ai_change_allow_observations, desktop_ai_change_start_review,
        desktop_sleep_source_status, desktop_sleep_configure_inbox, desktop_sleep_scan, desktop_sleep_history, desktop_sleep_delete_record
