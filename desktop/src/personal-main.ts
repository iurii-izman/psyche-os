...
const sleepSourceStateLabel = (value: string | null | undefined) =>
  (
    ({ ACTIVE: "Готов", DISABLED: "Не настроен", ERROR: "Нужна проверка" }) as Record<string, string>
  )[String(value)] ?? String(value ?? "Неизвестно");

const sleepSnapshotLabel = (value: string | null | undefined) =>
  (
    ({ COMPLETE: "Полный", PARTIAL: "Частичный" }) as Record<string, string>
  )[String(value)] ?? "Нет снимка";
...
export async function mountPersonal(
  ...
  const sleepMarkup = () => {
    ...
    const sampleMarkup = samples.length
      ? samples.map((sample) => `<article class="session-row"><div><strong>${escape(sleepMetricLabel(sample.metric))}</strong><small>${escape(sample.observed_at)} · ${escape(sample.value)} ${escape(sample.unit)}</small></div></article>`).join("")
      : "Показатели не были импортированы для этого интервала.";
    const provenance = sleepSource
      ? `<section class="card"><h2>Источник и полнота</h2><dl><dt>Источник</dt><dd>${escape(sleepSource.label ?? "Health.md")}</dd><dt>Состояние</dt><dd>${escape(sleepSourceStateLabel(sleepSource.state))}</dd><dt>Последний успешный импорт</dt><dd>${escape(sleepSource.last_imported_at ?? "ещё не было")}</dd><dt>Снимок</dt><dd>${escape(sleepSnapshotLabel(sleepSource.snapshot_status))}${sleepSource.snapshot_status === "PARTIAL" ? ` · замечаний: ${sleepSource.issue_count ?? 0}` : ""}</dd></dl>${sleepSource.snapshot_status === "PARTIAL" ? "<p class=\"derived-warning\"><strong>Неполные данные:</strong> часть сведений могла не попасть в импорт и не заменяется нулями.</p>" : ""}</section>`
      : "";
    const partialWarning = sleepSource?.snapshot_status === "PARTIAL"
      ? "<p class=\"derived-warning\"><strong>Неполные данные:</strong> отсутствующие стадии и показатели не означают нулевые значения.</p>"
      : "";
    return `<main class="product-shell">${nav()}<section class="product-home"><p>LOCAL HEALTH CONNECT</p><h1>Сон</h1><p>Стадии сна — оценки устройства/поставщика, а не медицинский вывод.</p></section>${latest ? `<section class="card"><h2>Последняя ночь · ${durationLabel(duration)}</h2><p>${escape(latest.started_at)} → ${escape(latest.ended_at)}</p>${partialWarning}<div class="review-counts">${Object.entries(stageSummary).map(([stage, value]) => `<span>${escape(sleepStageLabel(stage))} · ${durationLabel(value)}</span>`).join("") || "Стадии недоступны / частичны."}</div><section class="card"><h3>Хронология стадий</h3>${latest.stages.length ? latest.stages.map((stage) => `<p>${escape(sleepStageLabel(stage.category))} · ${escape(stage.started_at)} → ${escape(stage.ended_at)}</p>`).join("") : "Стадии недоступны / частичны."}</section><section class="card"><h3>Физиологические показатели во время сна</h3>${sampleMarkup}</section></section>` : `<section class="card calm-empty"><h2>Данных о сне пока нет</h2><p>Выберите локальную папку Health.md в настройках, затем проверьте её. Отсутствующие показатели не заменяются нулями.</p></section>`}<section class="card"><h2>Последние 14 ночей</h2>${sleepEpisodes.length ? sleepEpisodes.map(row).join("") : "Нет импортированных ночей."}</section>${provenance}${noticeMarkup()}</main>`;
  };
  const homeMarkup = () => {
    ...
    const reviewCard = activeChange && observationCount ? `<section class="card"><p>НАБЛЮДЕНИЯ · ЛОКАЛЬНО ПО УМОЛЧАНИЮ</p><h2>${observationCount} ${observationCount === 1 ? "запись" : "записей"}</h2><p>Для AI-разбора разрешено: ${eligibleObservationCount}. Будущие наблюдения не будут разрешены автоматически.</p>${eligibleObservationCount ? `<button class="primary" data-change-review="${escape(activeChange.plan_id)}">Разобрать с PSYCHE</button><button class="danger" data-change-observations-revoke="${escape(activeChange.plan_id)}">Отозвать разрешение</button>` : `<button class="primary" data-change-observations-allow="${escape(activeChange.plan_id)}">Разрешить наблюдения для AI-разбора</button>`}</section>` : "";
