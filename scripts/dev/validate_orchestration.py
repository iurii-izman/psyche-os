...
def validate_stable_gate_architecture(
    ...
    result.check(gate.get("opening_rule", {}).get("attestation_method") == "local_human_attestation_v1", "policy names the human attestation method")
    ...
    result.check(gate.get("authority", {}).get("profile_definition") == "docs/architecture/REAL_DATA_GATE_PROFILE.yaml", "policy profile owner path is canonical")
    result.check(gate.get("evaluation_records", {}).get("namespace_pattern") == "artifacts/e11/gate-evaluations/<evaluation-id>.yaml", "policy declares the exact evaluation namespace")
    result.check(gate.get("evaluation_records", {}).get("draft_can_support_open") is False and gate.get("evaluation_records", {}).get("sealed_content_immutable") is True, "policy requires DRAFT-to-SEALED fail-closed lifecycle")
    ...
    result.check(isinstance(profile.get("profile_id"), str) and isinstance(profile.get("profile_version"), str), "stable profile has versioned identity")
...
def validate_product_mode_policy(
    ...
    result.check(examples.get("accepted_storage_ipc_filesystem_or_permission_model_use") == "low_or_medium", "accepted mechanisms alone are LOW/MEDIUM")
    result.check(examples.get("material_storage_or_recovery_integrity_change") == "high", "material storage/recovery change is HIGH")
    result.check(examples.get("irreversible_migration_or_crypto_key_lifecycle_change") == "critical", "migration or crypto/key change is CRITICAL")
    result.check(risk.get("levels", {}).get("low", {}).get("initial_model") == "currently_selected_capable_coding_agent", "LOW uses the currently selected capable agent")
    result.check(risk.get("levels", {}).get("medium", {}).get("initial_model") == "currently_selected_capable_coding_agent", "MEDIUM uses the currently selected capable agent")
    ...
    result.check("REAL_DATA_GATE or real-data admission change" in approvals.get("require_explicit_approval", []), "real-data admission remains explicitly approved")
    result.check("material security architecture change" in approvals.get("require_explicit_approval", []), "material security architecture remains explicitly approved")
    ...
    result.check("material_permission_or_network_expansion" in review.get("triggers", []), "material permission/network expansion triggers review")
    ...
    result.check(selection.get("ordinary_product") == "targeted_tests_then_one_final_gate_or_exact_sha_pr_ci", "ordinary product verification permits relevant exact-SHA PR CI")
