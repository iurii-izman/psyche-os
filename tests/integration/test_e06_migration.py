...
def test_v4_checksum_and_exact_additive_inventory_preserve_prior_history() -> None:
    assert tuple(map(len, (V1_INVENTORY, V2_INVENTORY, V3_INVENTORY, V4_INVENTORY))) == (20, 35, 39, 43)
    ...
    assert V4_MIGRATION_CHECKSUM == "d5e372668458e7d1b4b9c096f3041c2f7185b3863a1ef1eec4d0a93535495faf"
...
def test_established_v3_requires_proofs_migrates_once_and_rerun_is_verified_noop() -> None:
    ...
    assert connection.execute("SELECT checksum FROM schema_migrations WHERE version=3").fetchone()[0] == V3_MIGRATION_CHECKSUM
...
def test_v4_interruption_rolls_back_to_exact_v3() -> None:
    ...
    try:
        MIGRATIONS[4] = Migration(4, original.label, [original.statements[0], "CREATE TABLE interrupted_e06(x INTEGER);", "INVALID SQL;"])
        report = Migrator(connection).apply(4, backup_verified=True, export_verified=True)
        assert not report.success
        assert connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] == 3
        assert {row[0] for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'")} == set(V3_INVENTORY)
    finally:
...
def test_v1_v2_v3_v4_logical_portability_remains_versioned() -> None:
    package = {}
    for version, inventory in ((1, V1_INVENTORY), (2, V2_INVENTORY), (3, V3_INVENTORY), (4, V4_INVENTORY)):
