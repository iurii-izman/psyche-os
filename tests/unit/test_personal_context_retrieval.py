...
def _seed(tmp_path: Path) -> tuple[ReflectionSessionService, str, str, str, str]:
    ...
    reflection = sessions.create_session("Synthetic reflection about a beacon")
    first = sessions.add_user_turn(
        reflection["session_id"], "Synthetic beacon glowed green in the morning"
    )
    second = sessions.add_user_turn(reflection["session_id"], "Later the synthetic beacon was red")
    ...
    sessions.connection.execute(
        "UPDATE reflection_context_items SET text='Synthetic uncertainty about a beacon.' WHERE context_item_id=?",
        (context_id,),
    )
    sessions.connection.execute(
    ...
    sessions.connection.execute(
        ...
        (
            ...
            "light",
            "CONTRADICTION",
            "Synthetic beacon colors conflict.",
            "UNRESOLVED",
    ...
    corrected = guided.correct_formulation(formulation["formulation_id"], "CORRECTION_ONLY_NEEDLE")
    guided.set_formulation_status(corrected["formulation_id"], "CURRENT")
    sessions.connection.execute(
        "UPDATE reflection_formulations SET summary=?, correction_text=? WHERE formulation_id=?",
        (
            "Synthetic corrected formulation without the unique marker.",
            "CORRECTION_ONLY_NEEDLE",
...
def test_retrieval_finds_source_derived_and_ai_with_exact_provenance(tmp_path: Path) -> None:
    ...
    view = PersonalContextRetrievalService(sessions).search(
        "synthetic", now=datetime(2026, 8, 27, tzinfo=UTC)
    )
...
def test_retrieval_filters_period_content_and_formulation_state(tmp_path: Path) -> None:
    ...
    current = retrieval.search(
        "synthetic", content="FORMULATION", formulation_status="CURRENT", now=now
    )
    assert [item["result_id"] for item in current["results"]] == [f"formulation:{current_id}"]
    recent = retrieval.search("beacon", period="7D", now=now)
    assert recent["results"] == []
    all_time = retrieval.search("beacon", period="ALL", now=now)
...
def test_retrieval_matches_correction_text_with_original_lineage_and_provenance(
    tmp_path: Path,
) -> None:
    sessions, session_id, turn_id, ai_id, current_id = _seed(tmp_path)
    view = PersonalContextRetrievalService(sessions).search(
    ...
    assert [item["result_id"] for item in view["results"]] == [f"formulation:{current_id}"]
    result = view["results"][0]
    assert result["result_type"] == "FORMULATION"
    assert result["ai_provenance"] is None
    assert result["session_id"] == session_id
    assert result["text"] == "Synthetic corrected formulation without the unique marker."
    assert result["correction_text"] == "CORRECTION_ONLY_NEEDLE"
    assert result["parent_result_id"] == f"formulation:{ai_id}"
    assert result["source_turns"][0]["turn_id"] == turn_id
    sessions.close()
