...
def request_for(identity=IDENTITY):
    return ProviderRequest(
        "request-1",
        "manifest-1",
        "synthetic_evidence_grounded_reflection",
        identity,
        (
            type(
                "R",
            ...
            type(
                ...
                (),
                {
                    ...
                    "content": "Fictional unknown.",
                },
            )(),
        ),
        "e07-reflection-proposal-v1",
        ClaimLevel.C0_OBSERVATION,
        "disabled",
    )
...
def valid():
    return {
        ...
        "reflections": [
            {
                "statement_id": "s1",
                "text": "The fictional amber report is selected evidence.",
                "supporting_evidence_ids": ["support-1"],
                "uncertainty": "The selected fictional records disagree.",
                "claim_level": 0,
            }
        ],
        "counterevidence": ["counter-1"],
        "unknowns": [
            {"unknown_id": "unknown-1", "uncertainty": "The fictional state remains unknown."}
        ],
        "questions": [
            {
                "question_id": "q1",
                "unknown_id": "unknown-1",
                "text": "Which fictional state was recorded?",
            }
        ],
    }


def manifest_for() -> ContextManifest:
    return ContextManifest.create(
        purpose="synthetic_evidence_grounded_reflection",
        records=(
            ManifestRecord(
                "support-1", "v1", "assertion", EvidenceRole.SUPPORTING, "policy-support"
            ),
            ManifestRecord(
                "counter-1", "v1", "assertion", EvidenceRole.COUNTEREVIDENCE, "policy-counter"
            ),
            ManifestRecord("unknown-1", "v1", "unknown", EvidenceRole.UNKNOWN, "policy-unknown"),
        ),
        cutoff=dt.datetime(2026, 8, 23, tzinfo=dt.UTC),
        policy_decision_id="policy-decision-1",
        provider_identity=IDENTITY,
        context_builder_version="test-v1",
        claim_ceiling=ClaimLevel.C0_OBSERVATION,
        action_ceiling="disabled",
    )
...
def test_request_is_exactly_pinned_and_schema_is_bounded():
    ...
    provider = OpenAIReflectionProvider(
        api_key="test-key",
        transport=lambda req, timeout: calls.append((req, timeout)) or Response(wrapped(valid())),
    )
    ...
    assert (
        body["store"] is False
        and body["model"] == "gpt-5.6-luna"
        and body["max_output_tokens"] == 900
    ...
    for requirement in ("support-1", "counter-1", "unknown-1", "claim_level must be exactly: 0"):
        assert requirement in instruction


@pytest.mark.parametrize(
    "identity",
    [ProviderIdentity("other", "gpt-5.6-luna", "x"), ProviderIdentity("openai", "other", "x")],
)
def test_missing_key_or_wrong_identity_never_calls_transport(identity):
    calls = []
    with pytest.raises(ProviderUnavailableError):
        OpenAIReflectionProvider(api_key="", transport=lambda *_: calls.append(1)).invoke(
...
def test_redirect_is_one_call_and_fails_closed(status):
    ...
    def transport(req, timeout):
        ...
        raise error.HTTPError(
            "https://api.openai.com/v1/responses", status, "redirect", {}, BytesIO()
        )

    with pytest.raises(ProviderUnavailableError):
        OpenAIReflectionProvider(api_key="test-key", transport=transport).invoke(request_for())
...
def test_http_error_keeps_only_bounded_safe_diagnostic_metadata():
    def transport(*_, **__):
        raise error.HTTPError(
            ...
            {},
            BytesIO(
                b'{"error":{"type":"invalid_request_error","code":"unsupported_schema","param":"text.format.schema","message":"secret content"}}'
            ),
        )

    provider = OpenAIReflectionProvider(api_key="test-key", transport=transport)
    with pytest.raises(ProviderUnavailableError):
        provider.invoke(request_for())
    assert provider.last_error_metadata == {
        "http_status": 400,
...
def test_network_failure_has_no_retry(failure):
    ...
    with pytest.raises(ProviderTimeoutError):
        OpenAIReflectionProvider(
            api_key="test-key",
            transport=lambda *_, **__: calls.append(1) or (_ for _ in ()).throw(failure),
        ).invoke(request_for())
    assert len(calls) == 1
...
@pytest.mark.parametrize(
    "body",
    [b"{", json.dumps({"output": []}).encode(), b"x" * 300_000],
    ids=["malformed-json", "missing-output", "oversized"],
)
def test_malformed_or_oversized_response_fails_closed(body):
    with pytest.raises(ProviderUnavailableError):
        OpenAIReflectionProvider(
            api_key="test-key", transport=lambda *_, **__: Response(body)
        ).invoke(request_for())


def test_valid_and_invalid_provider_json_still_go_through_local_authority():
    raw = OpenAIReflectionProvider(
        api_key="test-key", transport=lambda *_, **__: Response(wrapped(valid()))
    ).invoke(request_for())
    proposal = validate_provider_output(raw, manifest_for())
    assert proposal.status is ProposalStatus.PROPOSED
    invalid = valid()
    invalid["counterevidence"] = ["support-1"]
    with pytest.raises(ProposalValidationError) as exc_info:
        validate_provider_output(invalid, manifest_for())
    assert exc_info.value.code == "missing_or_invalid_counterevidence"
